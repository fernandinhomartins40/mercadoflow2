...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Extract + import Redetop Online catalog via VIPCommerce API")
    parser.add_argument("--max-pages", type=int, default=0)
    parser.add_argument("--max-records", type=int, default=0)
    parser.add_argument("--output", default="data/catalog/redetoponline_web_br_catalog")
...
def main() -> int:
    ...
    result = run_supermercados_online_catalog_job(
        job,
        options,
        max_pages_override=max(0, args.max_pages),
        max_records_override=max(0, args.max_records),
    )
