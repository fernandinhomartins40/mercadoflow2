...
export const marketService = {
  async getDashboard(marketId: string, startDate?: string, endDate?: string) {
    const response = await api.get(`/api/v1/markets/${marketId}/dashboard`, {
  ...
  async getProducts(marketId: string, page = 0, size = 20, category?: string) {
    const response = await api.get(`/api/v1/markets/${marketId}/products`, {
  ...
  async getAlerts(marketId: string, onlyUnread = false) {
    const response = await api.get(`/api/v1/markets/${marketId}/alerts`, {
      params: { onlyUnread },
    });
    return response.data;
  },
  ...
  async getTopSellers(marketId: string, limit = 10, startDate?: string, endDate?: string) {
    const response = await api.get(`/api/v1/markets/${marketId}/analytics/top-sellers`, {
