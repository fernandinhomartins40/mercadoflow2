...
const buildConnectionTestScript = (baseDir: string) => {
  return [
    ...
    '        result = {',
    '            "success": bool(heartbeat_ok),',
    '            "profileOk": True,',
    '            "online": bool(heartbeat_ok),',
    ...
    '            "watchPaths": len(watch_paths),',
    '            "title": "Conexão validada" if heartbeat_ok else "Heartbeat falhou",',
    '            "message": "API key validada e heartbeat confirmado." if heartbeat_ok else "A API key foi validada, mas o heartbeat do agente falhou.",',
    '        }',
    'except Exception as exc:',
    '    result = {',
    '        "success": False,',
